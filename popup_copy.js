...
function buildTypedUrlList(divName) {
  ...
  chrome.storage.local.get(
    ...
    function (result) {
      startTime = result.stopwatchStartTime;
      console.log("My inner start time", startTime);
      toggleStatus = result.isToggleOn;

      //let chromeStart = startTime ? startTime : oneWeekAgo;
      let chromeStart = startTime ? startTime : new Date();
      console.log("chrome start time", chromeStart);

      // Track the number of callbacks from chrome.history.getVisits()
      // that we expect to get.  When it reaches zero, we have all results.
      let numRequestsOutstanding = 0;

      //nested because fetch operations are async
      chrome.history.search(
        ...
        function (historyItems) {
          ...
          for (let i = 0; i < historyItems.length; ++i) {
            console.log("HISTRY LAUGHS", historyItems[i]);
      ...
      const processVisits = function (url, visitItems) {
        for (let i = 0, ie = visitItems.length; i < ie; ++i) {
          ...
          urlToCount[url].referringVisitIds.push(
            visitItems[i].referringVisitId
          );
        }
